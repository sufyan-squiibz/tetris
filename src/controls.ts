...
const lastMoveTime: { left: number; right: number; down: number } = {
  left: 0,
  right: 0,
  down: 0,
};
...
export function setupCustomControls(newControls: Partial<typeof CONTROLS>): void {
...
export function setSensitivity(value: number | string): void {
  ...
  localStorage.setItem('tetris-sensitivity', String(MOVE_DELAY));
