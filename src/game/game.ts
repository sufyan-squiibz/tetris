...
export class TetrisGame {
        ...
        if (linesCleared > 0) {
            ...
            if (this.particleSystem) {
                clearedRows.forEach(row => {
                    const color = this.currentPiece?.color || '#ffffff';
                    this.particleSystem.createLineExplosion(row, color);
                });
    ...
    draw(): void {
        // Convertir le board en format compatible (les couleurs sont des strings)
        const boardForRender: (number | string)[][] = this.board.map(row => 
            row.map(cell => cell === 0 ? 0 : String(cell))
        );
        this.renderer.renderBoard(boardForRender);
        
        if (this.currentPiece && !this.gameOver) {
            this.renderer.renderCurrentPiece(this.currentPiece);
            this.renderer.renderGhostPiece(this.currentPiece, boardForRender);
        }
    }
