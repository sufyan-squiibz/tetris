...
import { HighScore, GameConfig } from './types';
...
export class TetrisGame {
  ...
  constructor(canvas: HTMLCanvasElement, audioManager: AudioManager, particleSystem?: ParticleSystem) {
    ...
    this.renderer = new GameRenderer(canvas, GAME_CONFIG);
  ...
  initialize(): void {
    this.renderer.initApp(
      ...
      { cols: COLS, rows: ROWS, blockSize: BLOCK_SIZE }
    );
    this.init();
  }
  ...
  private lockPiece(): void {
    ...
    const shape = this.currentPiece.getShape();
    const colorHex = this.currentPiece.color;
    // Stocker la couleur comme nombre hexadécimal
    ...
    for (let y = 0; y < shape.length; y++) {
      for (let x = 0; x < shape[y].length; x++) {
        if (shape[y][x] !== 0) {
          ...
          if (boardY >= 0) {
            this.board[boardY][boardX] = colorNum;
          }
        }
      }
    }
  ...
  async saveScore(name: string, score: number, level: number, lines: number): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        const { showNotification } = await import('./themes');
