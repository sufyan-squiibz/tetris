...
export class TetrisGame implements GameControls {
    // État du jeu
    board: (number | string)[][];
    ...
    draw(): void {
        ...
        // Convertir le board en format compatible
        const boardForRender: (string | number)[][] = this.board.map(row => 
            row.map(cell => cell)
        );
        this.renderer.drawBoard(boardForRender);
