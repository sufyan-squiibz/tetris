...
import { drawPieceOnCanvas } from './renderer-utils';
...
export class TetrisGame {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.renderer = new GameRenderer(canvas, this.board);
  ...
  dropPiece(isManual: boolean = false): boolean {
    ...
    if (this.checkCollision()) {
      ...
      if (window.audioManager) {
        (window.audioManager as any).playSound('drop');
      }
      return true;
    }
    if (isManual && window.audioManager) {
      (window.audioManager as any).playSound('move');
    }
    return false;
  }
  ...
  rotatePiece(): boolean {
    ...
    if (window.audioManager) {
      (window.audioManager as any).playSound('rotate');
    }
    return true;
  }
    ...
    if (linesCleared > 0) {
      ...
      if (window.particleSystem) {
        clearedRows.forEach(row => {
          (window.particleSystem as any).createLineExplosion(row, this.currentPiece?.color);
        });
        
        if (linesCleared === 4) {
          (window.particleSystem as any).createTetrisExplosion(clearedRows, '#FFD700');
        }
      }
  ...
  updateScore(linesCleared: number): void {
    ...
    if (window.audioManager) {
      (window.audioManager as any).playSound('clear');
      if (this.level > previousLevel) {
        (window.audioManager as any).playSound('levelup');
        if (window.particleSystem) {
          (window.particleSystem as any).createLevelUpEffect();
        }
      }
    }
  }
  ...
  updateNextPieces(): void {
    for (let i = 0; i < 3; i++) {
      ...
      if (canvas && this.nextPieces[i]) {
        ...
        drawPieceOnCanvas(canvas, this.nextPieces[i], blockSize);
      }
    }
  }
  ...
  updateHoldDisplay(): void {
    ...
    if (canvas) {
      drawPieceOnCanvas(canvas, this.holdPiece, 25);
    }
  }
  ...
  showGameOver(): void {
    ...
    if (window.audioManager) {
      (window.audioManager as any).playSound('gameover');
    }
  }
  ...
  draw(): void {
    if (this.renderer && this.currentPiece) {
