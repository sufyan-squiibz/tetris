...
import { GameRenderer, COLS, ROWS } from './renderer';
...
export class TetrisGame {
    ...
    constructor(canvas: HTMLCanvasElement, particlesCanvas: HTMLCanvasElement) {
        ...
        // Mettre à jour le thème du renderer
        ...
        for (let y = 0; y < shape.length; y++) {
            for (let x = 0; x < shape[y].length; x++) {
                if (shape[y][x] !== 0) {
                    ...
                    if (boardY >= 0) {
                        // Convertir la couleur en nombre pour le board
    ...
    clearLines(): number {
        ...
        if (linesCleared > 0) {
            ...
            // Effets visuels
            if (this.particleSystem && this.currentPiece) {
                clearedRows.forEach(row => {
                    this.particleSystem!.createLineExplosion(row, this.currentPiece!.color);
                });
    ...
    updateStats(): void {
        ...
        if (linesEl) linesEl.textContent = String(this.lines);
        if (goalEl) goalEl.textContent = String(this.level * 10);
        if (comboEl) comboEl.textContent = String(this.combo);
        if (maxComboEl) maxComboEl.textContent = String(this.maxCombo);
        if (tetrisCountEl) tetrisCountEl.textContent = String(this.tetrisCount);
    ...
    private showGameOver(): void {
        ...
        if (gameOverEl) (gameOverEl as HTMLElement).style.display = 'flex';
        if (finalScoreEl) finalScoreEl.textContent = this.score.toLocaleString();
        if (finalLevelEl) finalLevelEl.textContent = String(this.level);
        if (finalLinesEl) finalLinesEl.textContent = String(this.lines);
        if (finalMaxComboEl) finalMaxComboEl.textContent = String(this.maxCombo);
        if (finalTetrisEl) finalTetrisEl.textContent = String(this.tetrisCount);
    ...
    draw(): void {
        ...
        if (this.currentPiece && !this.gameOver) {
            this.renderer.drawCurrentPiece(this.currentPiece, this.board);
        }
    }

    private updateRendererTheme(): void {
        this.renderer.updateTheme(
            this.themeManager.getBoardBgNumber(),
            this.themeManager.getGridColorNumber()
        );
    }
