...
import { Renderer, COLS, ROWS } from './render';
...
export class TetrisGame {
  ...
  nextPieces: TetrisPiece[] = []; // 3 pièces en preview
  holdPiece: TetrisPiece | null = null;
    ...
    for (let y = 0; y < shape.length; y++) {
      for (let x = 0; x < shape[y].length; x++) {
        if (shape[y][x] !== 0) {
          ...
          if (boardY >= 0) {
            // Convertir la couleur en nombre pour le stockage
  ...
  showComboDisplay(): void {
    ...
    if (display) {
      ...
      display.setAttribute('style', 'opacity: 1; animation: comboPopup 1s ease-out;');

      setTimeout(() => {
        display.setAttribute('style', 'opacity: 0;');
      }, 1000);
    }
  }
